...
레지스터는 우리가 생각하는 HDD or SSD와 같은 저장 매체가 아니라 **연산 대상 및 연산 결과**를 기억하는데 사용된다. 16bit가 한 번에 처리할 수 있는 양은 2의 16제곱 65536이고, 64비트는 2^64 18448744073709551616이 된다. 즉 bit가 클수록 많은 양의 데이터를 처리할 수 있기에 더 빠른 연산을 할 수 있다. 특징으로는 x86명령어가 x86_64에서 실행가능하지만 그 반대로는 불가능하다.
...
#### apt
-  기능이 조금 빈약하지만 제일 기본적인 인터페이스다.(**Low Level**)
