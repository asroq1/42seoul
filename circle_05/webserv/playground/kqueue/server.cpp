...
int main() {
    ...
    // 1. 서버 소켓을 하나 만들어 준다. PF_INET은 IPv4프로토콜 사용이고,
    // SOCK_STREAM은 TCP를 사용하는 소켓을 만든다는 뜻
    ...
    // 2. bind함수를 통해 소켓에 IP주소 및 포트를 할당한다.
    if (bind(server_socket, (struct sockaddr *)&server_addr,
             sizeof(server_addr)) == -1)
        exit_with_perror("bind() error\n" + string(strerror(errno)));
    // 3. listen을 통해 클라이언트의 연결 요청을 기다린다.
    if (listen(server_socket, 5) == -1)
        exit_with_perror("listen() error\n" + string(strerror(errno)));
    // 4. fcntl 함수를 호출해서 해당 소켓을 논블로킹 모드로 바꾼다. 이렇게 하면
    // 서버는 클라이언트로부터 데이터를 비동기적으로 읽고 쓸 수 있다.
    fcntl(server_socket, F_SETFL, O_NONBLOCK);

    // 5. kqueue를 호출해서 kqueue에 서버 소켓을 등록한다.
    int kq;
    if ((kq = kqueue()) == -1)
    ...
    change_events(change_list, server_socket, EVFILT_READ, EV_ADD | EV_ENABLE,
                  0, 0, NULL);
    ...
    while (1) {
        ...
        /*
                                  kevent는 클라이언트 소켓에서 발생하는 I/O
           이벤트를 모니터링 하기 위해 만든다. 리턴값은 아직 진행되지 않은
           이벤트 갯수를 리턴한다. change_list에 등록된 이벤트를 kqueu에
           적용하고, 새로 발생한 이벤트를 event_list에 저장한다.
